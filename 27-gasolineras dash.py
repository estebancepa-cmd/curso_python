...
def obtener_precios_carburantes(codigo_provincia,df):
    """
...
codigo_provincia = "08"
df = []
datos = obtener_precios_carburantes(codigo_provincia,df)
...
combustibles = ["Precio Gasolina 95 E5", "Precio Gasóleo A", "Precio Gasolina 95 E5 Premium"]
...
with tab1:
    ...
    if df_filtrado.empty:
        st.error("No se encontraron gasolineras en el rango indicado.")
        st.stop()

    # Mostrar mapa de gasolineras cercanas
    mostrar_gasoli_cercanas()
    ...
    for _, row in df_filtrado.head(5).iterrows():
        ...
        do[["Rótulo", "Municipio", "Dirección", tipo_combustible, "distancia_km"]].reset_index(drop=True).head(30)
